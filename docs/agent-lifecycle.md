...
Target frontend architecture is also gated. `12-target-frontend/source-file-manifest.json` must describe a stack-aware feature/shared/design-system app: Next.js App Router uses `src/app` for route wiring only, Vite + React Router uses `src/routes` plus `src/App.tsx`, `src/features/<screen-id>/screens` owns product screen composition, `src/features/<workflow>/patterns` owns workflow patterns, `src/shared/ui` and `src/shared/layout` own contract-bound reusable UI, `src/shared/api` and `src/shared/auth` own external boundaries, and `src/design-system` owns tokens. Product UI composition inside route files, stack-mismatched route files, or `archetype/` scaffold namespaces is drift.
