...
Target frontend architecture is also gated. `12-target-frontend/source-file-manifest.json` must describe a stack-aware feature/shared/design-system app: Next.js App Router uses `src/app` for route wiring only, Vite + React Router uses `src/routes` plus `src/App.tsx`, `src/features/<screen-id>/screens` owns product screen composition, `src/features/<workflow>/patterns` owns workflow patterns, `src/shared/ui` and `src/shared/layout` own contract-bound reusable UI, `src/shared/api` and `src/shared/auth` own external boundaries, and `src/design-system` owns tokens. When no explicit target stack is approved, Archetype defaults to Vite + React Router; Next.js is selected only from explicit Next/App Router stack evidence. Product UI composition inside route files, stack-mismatched route files, or `archetype/` scaffold namespaces is drift.

Visible actions are also gated. `03-experience-architecture/*.json` and `06-frontend-agent-contract/action-contracts.json` must emit screen/domain-specific action labels and targets. Bare `Filter`, `Export`, `Edit`, `Run`, `Save changes`, `Create first item`, `apply_filter`, `export_current_view`, or `open_edit_flow` are blockers because they invite generic inert controls.
