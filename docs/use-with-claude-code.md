...
Before approval, Claude Code starts from `agent-context/context-summary.json`, `agent-context/phase-bundles/draft-review.json`, and `agent-context/phase-bundles/contract-approval.json`, then reads only the draft artifacts those bundles name. It must not implement from a `draft_contract` package.

When implementation starts, Claude Code starts from `agent-context/phase-bundles/test-first.json`, `agent-context/phase-bundles/implementation.json`, and `agent-context/phase-bundles/verification.json`, then reads the required canonical artifacts named by those bundles, such as `spec/archetype-spec.json`, `test-first/test-first-contract.json`, `test-first/test-quality-standard.json`, `implementation-contract.md`, `experience/route-map.json`, `screens/screen-inventory.json`, `design-system/tokens.json`, and `frontend-agent-contract/implementation-rules.json`.
