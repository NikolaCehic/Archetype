...
The target app generated by `write-target` is feature/shared/design-system structured. Route files live under `src/app` and delegate to `src/features/<screen-id>/screens`; reusable contract-bound UI lives in `src/shared/ui`, shell primitives in `src/shared/layout`, external boundaries in `src/shared/api` and `src/shared/auth`, and tokens in `src/design-system`.
