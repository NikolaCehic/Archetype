...
- Prefer MCP tool `archetype_run_lifecycle` as the backing primitive for `$archetype` or `/archetype`; use its `nextAction` before falling back to separate intake/generate tools.
