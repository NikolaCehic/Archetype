...
function buildTopLevelManifest(pkg: ArchetypePackage): Record<string, unknown> {
  const artifacts = manifestArtifactsForPackage("canonical");
...
function buildPackageReadme(pkg: ArchetypePackage): string {
  const readOrder = artifactReadOrderForPackage("canonical").map((artifact, index) => `${index + 1}. Read \`${artifact}\`.`);
  return [
...
function buildGeneratedAgentsMd(): string {
  return [
    ...
    "18. `04-design-system/design-quality-gate.json`",
    "19. `04-design-system/shadcn-integration.json`",
    "20. `design-system/tokens.json`",
    "21. `design-system/component-contracts.json`",
    "22. `verification-plan.md`",
    "23. `lifecycle/final-readiness-report.md`",
    "",
    "## Rules",
    "",
    ...
    "- Enforce `04-design-system/design-quality-gate.json`; generic blue-gray SaaS output and untouched shadcn defaults are blockers.",
...
function buildCanonicalTokens(pkg: ArchetypePackage): Record<string, unknown> {
  return {
    ...
    selectedDirectionId: pkg.designSystem.selectedDirectionId,
    designQualityGate: "04-design-system/design-quality-gate.json",
    shadcnIntegration: "04-design-system/shadcn-integration.json",
...
function buildImplementationRules(pkg: ArchetypePackage): Record<string, unknown> {
  return {
    ...
    shadcnTailwindContract: {
      ...
      required_before_implementation: true,
      component_layering: pkg.designSystem.shadcnIntegration.component_layering
    },
    convergenceStandard: {
    ...
    forbiddenBehavior: [
      ...
      "Do not ship default blue-gray SaaS UI or untouched shadcn examples.",
      "Do not bypass 04-design-system/design-quality-gate.json.",
...
export function exportPackage(pkg: ArchetypePackage, outDir: string, options: ExportPackageOptions = {}): void {
  ...
  const agentControlPlane = buildAgentControlPlaneReport(pkg, "canonical_contract");
  ...
  const sessionArtifacts = buildSessionArtifacts({
    ...
    routeSource: pkg.experience.routeMap,
    manifestArtifactCount: manifestArtifactsForPackage("canonical").length
  });
