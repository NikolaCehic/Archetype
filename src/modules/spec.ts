...
function buildSpecJson(pkg: SpecInput): Record<string, unknown> {
  ...
  return {
    ...
    verification: {
      ...
      repair_task_queue_path: "10-revision/repair-task-queue.json",
      ...
      required_evidence: [
        ...
        "Repair tasks generated from verification failures before revision claims",
...
function buildSpecMarkdown(pkg: SpecInput, specJson: Record<string, unknown>): string {
  ...
  return [
    ...
    "- Verification failures must become repair tasks in `10-revision/repair-task-queue.json` before contract revision is claimed.",
