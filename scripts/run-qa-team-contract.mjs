...
const contractDriftQaRequirements = [
  "## Role",
  ...
  "## Failure Routing Matrix",
  "## Self-Review Checklist",
  "contract_drift_ready_for_qa_lead",
  "contract_drift_needs_repair",
  "contract_drift_blocked_missing_evidence",
  "contract_drift_blocked_stale_or_inconsistent_evidence",
  "contract_drift_blocked_unapproved_contract_revision",
  "contract_drift_blocked_repair_queue_mismatch",
  "Contract drift QA produces evidence, not vibes.",
  "patch implementation drift first",
  "revise the contract only when approved source evidence proves the canonical spec is wrong",
  "Specification by example",
  "new user-approved evidence",
  ...
  "type_contract_drift",
  "build_runtime_drift",
  "route_rendering_drift",
  "screen_state_drift",
  "flow_traceability_drift",
  "responsive_drift",
  "accessibility_drift",
  "visual_smoke_drift",
  "playwright_drift",
  "marker_only_test_drift",
  "malformed_data_drift",
  "accessibility_qa_drift",
...
for (const base of ["agents", path.join("plugins", "claude-code", "agents")]) {
  const contractDriftQa = readText(path.join(root, base, "contract-drift-qa.md"));
  for (const expected of contractDriftQaRequirements) {
    assert(contractDriftQa.includes(expected), `${base}/contract-drift-qa.md missing hardened contract drift QA requirement: ${expected}.`);
  }
}
