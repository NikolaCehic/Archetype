...
This is the user-facing front door. The user should be able to write a normal request and attach/import context with `@` references.
...
6. If ready or warning-only, read the generated contract entrypoints and continue with the implementation workflow from `archetype-implement`.
7. Verify the target with `archetype-verify`.
